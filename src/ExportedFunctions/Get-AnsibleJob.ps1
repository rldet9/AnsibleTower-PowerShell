function Get-AnsibleJob {
    ...
    process {
        ...
        foreach ($ResultObject in $Return) {
            ...
            if (!$ResultObject.started){$ResultObject.PSObject.Properties.Remove('started')}
            if (!$ResultObject.finished){$ResultObject.PSObject.Properties.Remove('finished')}
            $serializer = [System.Web.Script.Serialization.JavaScriptSerializer]::new()
            $AnsibleObject = $serializer.Deserialize((ConvertTo-Json ($ResultObject)), [Job])
